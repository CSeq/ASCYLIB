...
test_server(void* thread)
{
  ...
  queue_rpc_t* rpc = (queue_rpc_t*) aligned_alloc(CACHE_LINE_SIZE, sizeof(queue_rpc_t));
  assert(rpc != NULL);
  ssmp_color_buf_t* cbuf = (ssmp_color_buf_t*) aligned_alloc(CACHE_LINE_SIZE, sizeof(ssmp_color_buf_t));
  assert(cbuf != NULL);
